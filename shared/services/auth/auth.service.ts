...
export class AuthService {
  ...
  public login(cpf:string, password:string, rememberMe:boolean): Observable<boolean> {
    ...
    .pipe(map(res => {
        ...
        if (body.error_description == 'Usu&aacute;rio inexistente ou senha inv&aacute;lida' || body.error_description == 'Bad credentials') {
          errDesc = 'CPF inexistente ou senha inválida.';
        }
        else if (body.error_description == 'Usu&aacute;rio desabilitado') {
          errDesc = 'Usuário desabilitado';
        }
  ...
  public getAuthUserPage(idPage: number): Observable<Permission> {
    if ( this.authUser ) {
      ...
      // Se houver permissão na base de dados com o id igual ao passado na controller retorna.
      if(permssion !== null) {
        
        return of( permssion );

      } else {
        // Se não existir, instancia uma nova com as permissões como false, para que o front não quebre.
